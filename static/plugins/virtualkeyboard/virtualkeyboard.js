...
(function (factory) {
  ...
  $.fn.extend({
    'virtualkeyboard': function (options) {
      ...
      $(this).click((event) => {
        ...
        if($(event.target).attr("keyboard")=='no'|| ['password','text','number'].indexOf($(event.target).prop("type"))==-1)
        {
          return
        }
        const keyboard = $('.virtualkeyboard',window.top.document)
        if (!keyboard.length) {
          var $keyboardHTML=$(keyboardHTML);
          $keyboardHTML.click((e) => {
            ...
            switch (true) {
              ...
              case el.hasClass('slideDown'):
                ...
                $('.virtualkeyboard',window.top.document).hide()
                break
          ...
          $("body",window.top.document).append($keyboardHTML);

        }
        $('.virtualkeyboard',window.top.document).show()
  ...
  function updateZHWords (e) {
    $('.virtualkeyboard .word',window.top.document).remove()
    const inputVal = $('.virtualkeyboard .zh-pinyin-letter',window.top.document).text()
    if (!inputVal) return
    ...
    for (let zhWord of zhWordSet.values()) {
      count++
      var $liword=$('<li class="word">');
      $liword.addClass(`${count > defaults.num ? 'hide' : ''}`).text(zhWord);
      $('.output-ZH',window.top.document).append($liword);
    }
    // 处理备选汉字区域分页，默认显示第一页并更新页码信息
    if (count < defaults.num) {
      $('.virtualkeyboard .outputZoneDown',window.top.document).addClass('unclick')
    } else {
      $('.virtualkeyboard .outputZoneDown',window.top.document).removeClass('unclick')
    }
    defaults.page = 1
    $('.virtualkeyboard .outputZoneUp',window.top.document).addClass('unclick')
    pageUpdate(count)
  }
  ...
  function clearZhWords (e) {
    $('.virtualkeyboard .word',window.top.document).remove()
    $('.virtualkeyboard .zh-pinyin-letter',window.top.document).text('')
    $('.outputZone',window.top.document).hide()
  }
  ...
  function pressNumberOrLetterKey (e) {
    if (defaults.ZH) {
      ...
      $('.outputZone',window.top.document).show()
      const inputVal = $('.virtualkeyboard .zh-pinyin-letter',window.top.document)
  ...
  function backspace (e) {
    const key = $('.virtualkeyboard .zh-pinyin-letter',window.top.document)
    if (key.text()) {
  ...
  function switchInputLanguage () {
    ...
    if (defaults.ZH) {
      $('.inputCut',window.top.document).text('中文')
      $('.semicolon',window.top.document).text('：')
      $('.comma',window.top.document).text('，')
      $('.period',window.top.document).text('。')
    } else {
      clearZhWords()
      $('.inputCut',window.top.document).text('英文')
      $('.semicolon',window.top.document).text(':')
      $('.comma',window.top.document).text('/')
      $('.period',window.top.document).text('.')
    }
  ...
  function simulateCapsLock (e) {
    ...
    const letters = $('.virtualkeyboard .letter',window.top.document)
    if (defaults.upperCase) {
      letters.each(function (index, el) {
        $(el).text($(el).text().toUpperCase())
      })
      $('.toUpper',window.top.document).text('大写')
      clearZhWords()
    } else {
      letters.each(function (index, el) {
        $(el).text($(el).text().toLowerCase())
      })
      $('.toUpper',window.top.document).text('小写')
    }
  }

  // 拼音输入时翻到下一页
  function pageUp (e) {
    const list = $('.virtualkeyboard .word',window.top.document)
    if (defaults.page > 1) {
      defaults.page--
  ...
  function pageUpdate (count) {
    $('.virtualkeyboard .page',window.top.document).text(defaults.page + '/' + (Math.ceil(count / defaults.num) || '1'))
  }
  ...
  function move (e) {
    e.preventDefault()
    const event = e.type.match('mouse') ? e : e.originalEvent.changedTouches[0]
    $('.virtualkeyboard',window.top.document)[0].style.left = (event.pageX - defaults.offsetX) + 'px'
    $('.virtualkeyboard',window.top.document)[0].style.top = (event.pageY - defaults.offsetY) + 'px'
  }
